...
ngOnInit():void{
  this.passwordForm = this.formBuilder.group({
    newPassword :[null, Validators.compose([Validators.required, CustomValidators.patterValidator(/[0-9]/,{hasNumber:true})])],
    confirmPassword:[null, Validators.compose([Validators.required])],
  });
}

get getFromControl(): {[key:string]:AbstractControl}{
return this.passwordForm.controls;
}
  

  onSubmit() {
  this.userService.setPassword(this.passwordForm.value).subscribe( res => {
    if (res && res.token) {
